...
class TestAllreducePersistent(unittest.TestCase):
    ...
    def _test(self, comm, color_value, model):
        print("color_value : {} / comm.intra_rank : {}".format(color_value,comm.intra_rank))
        rank = comm.rank
    ...
    def test_allreduce_persistent_gpu(self):
        ...
        model = ExampleModel()
        model.to_gpu()
        self._test(scomm, color_value, model)
        # self._test(wcomm, color_value, model)
